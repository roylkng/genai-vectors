...
async fn get_or_create_index_config(
    ...
    match s3.get_object(&config_key).await {
        ...
        Err(e) => {
            tracing::warn!(
                "Failed to load index config: {}, creating optimized config based on dataset characteristics",
                e
            );
            let estimated_total_vectors =
                estimate_total_dataset_size(s3, index_name, dimension * 100).await;
